...
class AutomationSetupDialog(QDialog):
    ...
    def __init__(self, project_name: str, project_path: str, parent=None):
        ...
        self.setWindowTitle(f"🚀 Tam Otomatik İşlem — {project_name}")
    ...
    def _build_ui(self):
        ...
        # Başlık
        title = QLabel("🚀 Tam Otomatik İşlem (Workflow)")
        title.setFont(QFont("Segoe UI", 13, QFont.Weight.Bold))
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet("color: #FFB74D; margin-bottom: 4px;")
        layout.addWidget(title)

        subtitle = QLabel("Her aşamayı checkbox ile etkinleştirebilir veya atlayabilirsiniz.")
    ...
    def _build_download_stage(self, parent_layout):
        self.download_check = QCheckBox("Aşama 1: İndirme")
        ...
        self.download_method_combo.addItems([
        ...
        dl_form.addRow("İndirme Yöntemi:", self.download_method_combo)

        # Bölüm sayısı (sadece Booktoki'de görünür)
        self.chapter_limit_label = QLabel("İndirilecek Bölüm:")
    ...
    def _build_prompt_stage(self, parent_layout):
        self.prompt_check = QCheckBox("Aşama 2: Prompt Üretme")
        ...
        # Prompt tipi seçimi
        type_label = QLabel("Ön Prompt Tipi:")
        type_label.setFont(QFont("Segoe UI", 9))
        pm_layout.addWidget(type_label)

        self.prompt_type_group = QButtonGroup(self)
        type_layout = QHBoxLayout()
        self.radio_literal = QRadioButton("📖 Birebir (A)")
        self.radio_natural = QRadioButton("💬 Doğal (B)")
        self.radio_balanced = QRadioButton("⚖️ Dengeli (C)")
        ...
        api_form.addRow("API / MCP:", self.prompt_api_combo)
    ...
    def _build_terminology_stage(self, parent_layout):
        self.terminology_check = QCheckBox("Aşama 3: Terminoloji Çıkarma")
        ...
        if self._chapter_count > 0:
            ...
            self.term_end_spin.setSuffix(f"  (Mevcut: {self._chapter_count})")
        else:
            self.term_end_spin.setValue(50)
            self.term_end_spin.setSuffix("  (Henüz dosya yok)")
        term_form.addRow("Son Bölüm:", self.term_end_spin)

        # API/MCP seçimi
        self.term_api_combo = QComboBox()
        self._populate_api_combo(self.term_api_combo)
        term_form.addRow("API / MCP:", self.term_api_combo)
    ...
    def _build_translation_stage(self, parent_layout):
        self.translation_check = QCheckBox("Aşama 4: Çeviri")
        ...
        self.translate_all_radio = QRadioButton("Tamamını Çevir")
        self.translate_all_radio.setChecked(True)
        self.translate_limit_radio = QRadioButton("Sayılı Çevir:")
        ...
        detail_form.addRow("API / MCP:", self.trans_api_combo)

        # Asenkron thread sayısı
        async_layout = QHBoxLayout()
        self.async_check = QCheckBox("Asenkron Çeviri")
        ...
        self.async_spin.setSuffix(" thread")
        ...
        self.batch_check = QCheckBox("Toplu Çeviri (Batch)")
        ...
        self.batch_chars_spin.setSuffix(" karakter")
    ...
    def _populate_api_combo(self, combo: QComboBox):
        ...
        if project_mcp:
            combo.addItem("📌 Proje MCP Varsayılanı", {"type": "project_mcp", "id": project_mcp})
        elif project_api:
            combo.addItem("📌 Proje API Varsayılanı", {"type": "project_api", "key": project_api})
    ...
    def _on_start(self):
        ...
        if not any([
            ...
            QMessageBox.warning(self, "Aşama Seçilmedi", "Lütfen en az bir aşamayı etkinleştirin.")
            return
        self.accept()
    ...
    def get_config(self) -> dict:
        ...
        # İndirme yöntemi → site key belirleme
        dl_method_text = self.download_method_combo.currentText()
        if "Booktoki" in dl_method_text:
            download_site = "booktoki"
        elif "69shuba" in dl_method_text:
            download_site = "shuba"
        elif "Novelfire" in dl_method_text:
            download_site = "novelfire"
        else:
            download_site = "booktoki"
