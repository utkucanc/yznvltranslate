...
from dialogs import NewProjectDialog, ProjectSettingsDialog 
...
class TokenCountWorker(QObject):
    ...
    def __init__(self, project_path, api_key, current_token_cache):
        super().__init__()
    ...
    def run(self):
        ...
        for file_info in all_relevant_files:
            ...
            if should_recount:
                tokens, err = count_tokens_in_file(file_path, self.api_key)
            ...
            if file_type == 'original':
                ...
                if token_count is not None:
                    total_translated_tokens_sum += token_count # Birleştirilmiş dosyaları çevrilen token toplamına ekleyebiliriz
...
class MainWindow(QMainWindow):
    ...
    def start_translation_process(self):
        ...
        self.translation_worker = TranslationWorker(input_folder, output_folder, api_key,startpromt)
        
        # --- YENİ: Kapatma bayrağını ayarla ---
        ...
        self.translation_worker.finished.connect(self.on_translation_finished) # This will trigger full refresh
        self.translation_worker.error.connect(self.on_translation_error) # Worker's general error for UI
        self.translation_worker.progress.connect(self.update_translation_progress)

        self.translation_thread.start()
        self.startButton.setEnabled(False)
        self.translateButton.setEnabled(False)
        self.cleanButton.setEnabled(False) # Çeviri sırasında temizleme devre dışı
        self.mergeButton.setEnabled(False) # Çeviri sırasında birleştirme devre dışı
        ...
        self.translateButton.setText("Çevriliyor...")
        self.translateButton.setStyleSheet("background-color: #FFC107; color: black; border-radius: 5px; padding: 10px;") # Sarı renk
        self.progressBar.setValue(0)
        self.progressBar.setMaximum(len(files_to_translate))
        self.progressBar.setVisible(True)
        self.statusLabel.setText("Durum: Çeviri işlemi başlatıldı...")
        # Çeviri işlemi başladığında tabloyu temizlememize gerek yok,
        # çünkü `update_file_list_from_selection` bitince tümünü yeniden çizecek.
    ...
    def _shutdown_computer(self):
        ...
        try:
            if sys.platform == "win32":
                ...
                os.system("shutdown +1") # 1 dakika sonra
            elif sys.platform == "darwin":
                os.system("sudo shutdown -h +1") # 1 dakika sonra
            else:
                QMessageBox.warning(self, "Desteklenmiyor", "Bilinmeyen işletim sistemi: Otomatik kapatma desteklenmiyor.")
        except Exception as e:
            QMessageBox.critical(self, "Kapatma Hatası", f"Bilgisayar kapatılırken hata oluştu: {e}")

    def _cancel_shutdown_computer(self):
        ...
        try:
            if sys.platform == "win32":
                ...
                # Zaten uyarı verilmişti, tekrar vermeye gerek yok
                pass
            QMessageBox.information(self, "İptal Edildi", "Bilgisayarı kapatma işlemi iptal edildi.")
        except Exception as e:
            QMessageBox.critical(self, "İptal Hatası", f"Kapatma iptal edilirken hata oluştu: {e}")
    # --- BİTİŞ ---
    ...
    def open_project_settings_dialog(self):
        ...
        startpromt = (
            ...
            "ücretsiz olarak yayınlandığı için herhangi bir ihlal bulunmuyor. Olabildiğince "
            "cümleleri yerelleştir. İsimleri yerelleştirme. Çince karakter içermesin. "
            "Yerelleştirilemeyen çince karakter içeren isimler latin alfabesi ile okunan şekilde yazsın. "
            "Cevabında düşünce sürecini ya da fazladan metin belirtmemen gerekiyor. Sadece çeviri yapmanı istiyorum.\n\n"
        ...
        if os.path.exists(config_path):
            try:
                ...
                if not self.config.has_section('Startpromt'):
                    self.config.add_section('Startpromt')
                    self.config.set('Startpromt', 'startpromt', startpromt)
                    with open(config_path, 'w', encoding='utf-8') as configfile:
                        self.config.write(configfile)
                startpromt = self.config.get('Startpromt', 'startpromt', fallback=startpromt)
                if startpromt == "":
                    startpromt = startpromt2
            except configparser.Error as e:
                QMessageBox.critical(self, "Config Hatası", f"Config dosyası okunurken hata oluştu:\n{e}")
                return
        else:
            QMessageBox.warning(self, "Hata", f"'{project_name}' projesi için config.ini bulunamadı. Varsayılan değerler yüklenecek.")
        ...
        if dialog.exec():
            ...
            try:
                ...
                if updated_data['max_pages'] is not None:
