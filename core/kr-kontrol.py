...
def klasoru_tara(klasor_yolu, kaynak_klasor_yolu=None, source_lang="ko"):
    """
    Belirtilen klasördeki txt dosyalarını bulur, kalite ve Korece/CJK karakter analizini raporlar.
    """
    if not os.path.exists(klasor_yolu):
        app_logger.error(f"Hata: '{klasor_yolu}' yolu bulunamadı.")
        return []

    app_logger.info(f"--- '{klasor_yolu}' klasörü taranıyor --- Çeviri/Korece Kontrol ---")
...
if __name__ == "__main__":
    ...
    dosya_sil(klasoru_tara(hedef_klasor, kaynak_klasor, source_lang="ko"))